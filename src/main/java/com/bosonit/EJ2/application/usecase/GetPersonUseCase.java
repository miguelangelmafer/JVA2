...
public class GetPersonUseCase implements GetPersonPort {
    ...
    {
        PersonaEnt persona = personaRepository.findById(id).orElseThrow(()-> new NotFoundException("Usuario no encontrado")) ;
        OutPutPersonaDTO outPutPersonaDTO = new OutPutPersonaDTO(persona.getId_persona(), persona.getUsuario(), persona.getPassword(), persona.getName(), persona.getSurname(), persona.getCompany_email(), persona.getPersona_email(), persona.getCity(), persona.getActive(), persona.getCreated_date(), persona.getImagen_url(), persona.getTermination_date());
